...
def trackMood():
    ...
    moodLabel = ctk.CTkLabel(dashboard, text="Mood Tracker")
    ...
    def saveMood():
        ...
        if moodTrack > 5:
            ...
            moodLabel.configure(text="Things will get better", fg_color="red")
            goalAchieved = 1
        else:
            moodLabel.configure(text="Great!", fg_color="green")
            goalAchieved = 1
...
def trackFood():
    ...
    def checkListOptions():
        ...
        foodLabel.configure(text="Food groups logged successfully!")
        ctk.CTkLabel(dashboard, text="Remember: Try to eat at least two meals with all of these daily!").pack(pady=5)
...
def trackWater():
    ...
    waterLabel = ctk.CTkLabel(dashboard, text="Water Tracker")
    ...
    def saveWater():
        ...
        if waterAmount > 2.7:
            waterLabel.configure(text="You have drunk the required amount for today")
            goalAchieved = 1
        else:
            waterLabel.configure(text="You should drink more water")
            goalAchieved = 0
        ...
        ctk.CTkLabel(dashboard, text="Water logged successfully!", fg_color="green").pack(pady=5)
    ...
    waterValueLabel = ctk.CTkLabel(dashboard, text="Water: 0.00 L", font=("Aptos", 14))
...
def trackSleep():
    ...
    def saveSleep():
        ...
        if sliderValue > 7:
            goalAchieved = 1
            sleepLabel.configure(text="You have slept enough!", fg_color="green")
        else:
            goalAchieved = 0
            sleepLabel.configure(text="Get more sleep!", fg_color="red")
    ...
    def saveExercise():
        ...
        try:
            ...
            if choice not in exerciseTypes:
                ctk.CTkLabel(dashboard, text="Please select a valid exercise type.", fg_color="red").pack(pady=5)
                return
            ...
            try:
                ...
                ctk.CTkLabel(dashboard, text=str(ve), fg_color="red").pack(pady=5)
                return
            ...
            exerciseLabel.configure(text=f"Exercise logged successfully!", fg_color="green")
        except Exception as e:
            ctk.CTkLabel(dashboard, text=f"Error: {e}", fg_color="red").pack(pady=5)
...
def doBadgeSystem():
    ...
    for widget in dashboard.winfo_children():                    # Clear the frame
        widget.destroy()
    badgeLabel= ctk.CTkLabel(dashboard, text="Congratulations!\nYou earned a badge!", fg_color="green", font=("Aptos", 16))
    badgeLabel.pack(pady=200)
...
def openMoodJournal():
    for widget in dashboard.winfo_children():                    # Clear the frame
        widget.destroy()
    journalLabel = ctk.CTkLabel(dashboard, text="Please write anything you'd like to remember from today!\nPlease make sure to press 'save' before exiting.", font=("Aptos", 16))
    ...
    def saveEntry():
        ...
        journalLabel.configure(text="Entry saved successfully!")
